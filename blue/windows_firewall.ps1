...
# Function to block all ports and allow specific ones - MODIFIED to accept allowed ports as parameter
function Block-All-And-Allow-Specific-Ports {
    param (
        [array]$allowedPorts # Array of hashtables defining allowed ports
    )
    ...
    # Allow DNS resolver - always allow DNS resolution for outbound
...
function Configure-Firewall-For-Host {
    param (
        [string]$HostName
    )

    Write-Host "Configuring firewall for host: $HostName" -ForegroundColor Yellow
    ...
    switch ($HostName.ToLower()) {
        ...
        "office" { # Office; Windows Server 2022; HTTPS (HTTPS)
            $allowedPorts = @(
                @{Port=443; Protocol="TCP"; Name="HTTPS"}
            )
        }
        "teller" { # Teller; Windows 10; WinRM (WinRM HTTP/HTTPS - using default HTTP 5985 and HTTPS 5986)
            $allowedPorts = @(
                @{Port=5985; Protocol="TCP"; Name="WinRM-HTTP"},
                @{Port=5986; Protocol="TCP"; Name="WinRM-HTTPS"}
            )
        }
        "safe" { # Safe; Windows Nano Server; Telnet, RDP (Telnet 23, RDP 3389)
            $allowedPorts = @(
        ...
        default {
            Write-Host "Invalid host name: $HostName. Please select from Wire, Vault, Office, Teller, Safe." -ForegroundColor Red
            return
        }
    }

    Write-Host "Allowed ports for $HostName" -ForegroundColor Green
    foreach ($port in $allowedPorts) {
        Write-Host "- $($port.Name) ($($port.Protocol)/$($port.Port))"
    }

    Block-All-And-Allow-Specific-Ports -allowedPorts $allowedPorts
}
...
function Main-Menu {
    ...
    while ($true) {
        ...
        Write-Host "1) Block all and allow essential ports (Default)"
        ...
        Write-Host "6) Exit"
        Write-Host "7) Configure firewall for a specific host (Wire, Vault, Office, Teller, Safe)" # New option
        $choice = Read-Host "Enter your choice"

        switch ($choice) {
            1 { # Original Block-All-And-Allow-Specific-Ports functionality - still useful as a default setup
                $defaultAllowedPorts = @(
                    @{Port=80; Protocol="TCP"; Name="HTTP"},
                    @{Port=443; Protocol="TCP"; Name="HTTPS"},
                    @{Port=53; Protocol="TCP"; Name="DNS-TCP"},
                ...
                Block-All-And-Allow-Specific-Ports -allowedPorts $defaultAllowedPorts
             }
            ...
            7 { # New option to configure for specific host
                $hostName = Read-Host "Enter the host name (Wire, Vault, Office, Teller, Safe)"
                Configure-Firewall-For-Host -HostName $hostName
            }
            default {
